...
public:
   ...
   std::string to_string() const
   {
      ...
      {
         bit_string += (bit_table_[i] ? '1' : '0');
      }
      return bit_string;
   }

   void from_string(const std::string& bit_string)
   {
      bit_table_.resize(bit_string.size());
      for (std::size_t i = 0; i < bit_string.size(); ++i)
      {
         bit_table_[i] = (bit_string[i] == '1' ? 1 : 0);
      }
   }
   ...
   inline virtual unsigned long long int size() const
   {
    ...
    inline virtual unsigned long long int salt_count() const
   {
      return salt_count_;
   }
